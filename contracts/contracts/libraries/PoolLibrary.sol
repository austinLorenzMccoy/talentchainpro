...
library PoolLibrary {
    ...
    function calculateMatchScore(
        ...
        require(requiredSkills.length == minimumLevels.length, "PoolLibrary: invalid required skills");
        require(candidateSkills.length == candidateLevels.length, "PoolLibrary: invalid candidate skills");
        
        uint256 totalPossibleScore = requiredSkills.length * MATCH_SCORE_BASE;
        ...
        for (uint256 i = 0; i < requiredSkills.length; i++) {
            for (uint256 j = 0; j < candidateSkills.length; j++) {
                if (keccak256(bytes(requiredSkills[i])) == keccak256(bytes(candidateSkills[j]))) {
                    uint256 skillScore = SKILL_WEIGHT;
                    
                    // Bonus for exceeding minimum level
                    if (candidateLevels[j] >= minimumLevels[i]) {
                        skillScore += LEVEL_WEIGHT;
                        
                        // Additional bonus for exceeding requirements
                        uint256 levelBonus = (candidateLevels[j] - minimumLevels[i]) * 10;
                        skillScore += levelBonus;
                    } else {
                        // Penalty for not meeting minimum level
                        uint256 penalty = (minimumLevels[i] - candidateLevels[j]) * 20;
    ...
    function generatePoolMetrics(
        ...
        uint256 averageTimeToFill = isCompleted ? 
            block.timestamp - createdAt : 0;
        
        return ITalentPool.PoolMetrics({
